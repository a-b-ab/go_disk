...
func processAutoTag(fileID, userID string) error {
	...
	// 注意：FilePath 当前存的是“云端对象所在的用户目录前缀”（用于秒传/去重场景），不能用 Owner。
	downLoadURL, err := disk.BaseCloudDisk.GetDownloadPresignedURL(file.FilePath, "", fileName)
	if err != nil {
...
func processFileClean(fileUuid string) error {
	var file model.File
	// 读取必要字段：owner/file_path 用于删除对象；file_uuid 用于对齐参数
	if err := model.DB.Select("owner", "file_path", "file_uuid").Where("file_uuid = ?", fileUuid).First(&file).Error; err != nil {
	...
	// 从云存储删除物理文件
	if err := disk.BaseCloudDisk.DeleteObject(file.Owner, file.FilePath, []string{fileUuid}); err != nil {

...
// FileRefCountService 文件引用计数服务
type FileRefCountService struct{}

// IncreaseRefCount 增加文件引用计数（转存、分享等场景）
func (service *FileRefCountService) IncreaseRefCount(fileUuid string) error {
	return model.DB.Model(&model.File{}).Where("file_uuid = ?", fileUuid).UpdateColumn("ref_count", gorm.Expr("ref_count + ?", 1)).Error
}

// DecreaseRefCount 减少文件引用计数（删除文件场景）
func (service *FileRefCountService) DecreaseRefCount(fileUuid string) error {
	return model.DB.Model(&model.File{}).Where("file_uuid = ?", fileUuid).UpdateColumn("ref_count", gorm.Expr("ref_count - ?", 1)).Error
}

// GetRefCount 获取文件引用计数
func (service *FileRefCountService) GetRefCount(fileUuid string) (int64, error) {
	var file model.File
	err := model.DB.Select("ref_count").Where("file_uuid = ?", fileUuid).First(&file).Error
	if err != nil {
		return 0, err
	}
...
func (service *FileRefCountService) LogicalDeleteFile(userID, fileID string) serializer.Response {
	...
	logger.Log().Info(fmt.Sprintf("[LogicalDeleteFile] 文件已移到回收站: FileID=%s, UserID=%s, RefCount=%d", fileID, userID, updatedFile.RefCount))
